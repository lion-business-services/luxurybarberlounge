...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: current } = await context.admin.from("barber_profiles").select("id,staff_user_id,display_name,professional_title,short_intro,biography,specialties,languages,active,featured,status,square_team_member_id").eq("business_id", context.businessId).eq("id", id).maybeSingle();
  ...
  if (current.staff_user_id && parsed.data.active !== undefined) await context.admin.from("staff_profiles").update({ active: parsed.data.active }).eq("business_id", context.businessId).eq("user_id", current.staff_user_id);
  await context.admin.from("audit_logs").insert({ business_id: context.businessId, actor_user_id: context.session.user.id, action: "barber_profile_updated", resource_type: "barber_profile", resource_id: id, before_data: current, after_data: update, metadata: {} });
