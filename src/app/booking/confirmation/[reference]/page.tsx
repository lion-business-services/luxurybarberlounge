...
export default async function BookingConfirmationPage({ params, searchParams }: { params: Promise<{ reference: string }>; searchParams: Promise<{ token?: string }> }) {
  ...
  const awaitingDeposit = remainingCents > 0 && appointment.status === "pending_confirmation";
  const paymentAmount = (remainingCents / 100).toLocaleString("en-US", { style: "currency", currency: "USD" });

  return <main className="min-h-screen bg-[var(--color-ink)] px-5 py-16 text-[var(--color-bone)] sm:px-8"><section className="mx-auto max-w-3xl border border-[var(--color-brass)]/30 bg-[var(--color-ink-soft)] p-7 sm:p-12">{awaitingDeposit ? <Clock className="h-10 w-10 text-amber-400" /> : <CheckCircle2 className="h-10 w-10 text-[var(--color-brass)]" />}<p className={`mt-6 text-[10px] tracking-[.3em] uppercase ${awaitingDeposit ? "text-amber-400" : "text-[var(--color-brass)]"}`}>{awaitingDeposit ? "Payment required · not yet confirmed" : "Appointment confirmed"}</p><h1 className="font-display mt-3 text-4xl sm:text-6xl">{awaitingDeposit ? "Almost there." : "Your chair is reserved."}</h1>{awaitingDeposit ? (
      <>
        <p className="mt-5 text-sm leading-7 text-[var(--color-bone-muted)]">We are holding this time slot for you, but <strong className="text-amber-300">your appointment is not confirmed yet</strong>. Pay {paymentAmount} below (plus a 4% service fee) to complete the required service payment.</p>
        ...
        <div className="mt-6 flex items-start gap-3 rounded-xl border border-amber-400/40 bg-amber-400/10 p-4">
          ...
          <p className="text-sm leading-6 text-amber-100">Full service payment is required before a website appointment is confirmed. You will not be charged again for service principal already paid.</p>
        </div>
