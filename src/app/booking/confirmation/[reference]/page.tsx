...
import { CalendarPlus, CheckCircle2, MapPin, Phone, Scissors, UserRound } from "lucide-react";
...
export default async function BookingConfirmationPage({ params, searchParams }: { params: Promise<{ reference: string }>; searchParams: Promise<{ token?: string }> }) {
  ...
  return <main className="min-h-screen bg-[var(--color-ink)] px-5 py-16 text-[var(--color-bone)] sm:px-8"><section className="mx-auto max-w-3xl border border-[var(--color-brass)]/30 bg-[var(--color-ink-soft)] p-7 sm:p-12"><CheckCircle2 className="h-10 w-10 text-[var(--color-brass)]" /><p className="mt-6 text-[10px] tracking-[.3em] uppercase text-[var(--color-brass)]">Appointment saved</p><h1 className="font-display mt-3 text-4xl sm:text-6xl">Your chair is reserved.</h1><p className="mt-5 text-sm leading-7 text-[var(--color-bone-muted)]">Keep your reference number. Confirmation and reminder delivery continue in the background even if an email provider needs a retry.</p><div className="mt-8 rounded-xl border border-[var(--color-ink-line)] p-5"><p className="text-[9px] tracking-[.2em] uppercase text-[var(--color-brass)]">Booking reference</p><p className="font-display mt-2 text-3xl">{appointment.public_reference}</p></div><dl className="mt-7 grid gap-4 sm:grid-cols-2"><Detail icon={<Scissors />} label="Service" value={appointment.service_name_snapshot} /><Detail icon={<UserRound />} label="Barber" value={appointment.barber_name_snapshot} /><Detail icon={<CalendarPlus />} label="Date and time" value={date} /><Detail icon={<MapPin />} label="Location" value={address} /></dl><div className="mt-8 flex flex-wrap gap-3"><a href={calendarPath} className="inline-flex min-h-12 items-center gap-2 rounded-full bg-[var(--color-brass)] px-5 text-[10px] tracking-[.16em] uppercase text-black"><CalendarPlus className="h-4 w-4" />Download calendar</a><a href={google.toString()} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center rounded-full border border-[var(--color-ink-line)] px-5 text-[10px] tracking-[.16em] uppercase">Google Calendar</a><a href={businessConfig.mapsUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center rounded-full border border-[var(--color-ink-line)] px-5 text-[10px] tracking-[.16em] uppercase">Directions</a><a href={businessConfig.phoneHref} className="inline-flex min-h-12 items-center gap-2 rounded-full border border-[var(--color-ink-line)] px-5 text-[10px] tracking-[.16em] uppercase"><Phone className="h-4 w-4" />Call</a></div><GuestAppointmentActions reference={reference} token={token} startsAt={appointment.starts_at} status={appointment.status} /><div className="mt-8 flex flex-wrap gap-5 text-sm"><Link href="/login?next=/client/appointments" className="text-[var(--color-brass)] underline underline-offset-4">Access your client portal</Link><Link href="/book" className="text-[var(--color-bone-muted)] underline underline-offset-4">Book another appointment</Link></div></section></main>;
}
