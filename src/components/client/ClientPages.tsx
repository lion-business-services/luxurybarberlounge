...
export async function ClientProfilePage() {
  ...
  return <ClientPageHeader eyebrow="Your details" title="Profile" copy="Identity, contact, language, and grooming preferences for your account.">
    <div className="grid gap-4 md:grid-cols-2">
      <section className={styles.card}><h2 className="font-display text-2xl">Verified account</h2><div className="mt-5 grid gap-4"><Field label="Email" value={data.profile.email ?? "—"} /><Field label="Account status" value={titleCase(data.profile.status)} /><Field label="Square customer link" value={data.clientProfile?.squareCustomerId ? "Connected" : "Not linked yet"} /><Field label="Marketing preference" value={titleCase(safeMarketing)} /></div><p className={`mt-5 text-xs leading-5 ${styles.muted}`}>Email changes require a new verified authentication flow. Square matching never merges by name alone.</p></section>
