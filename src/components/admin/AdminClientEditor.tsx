...
export function AdminClientEditor({ client }: { client: AdminClientDetailData }) {
  ...
  return <div className="grid gap-4 xl:grid-cols-2">
    ...
    <section className="portal-card"><h2 className="font-display text-2xl">Internal history</h2>{!client.authUserId ? <p className="mt-3 text-xs leading-5 text-[var(--color-bone-muted)]">This is a guest booking client. Identity fields are fully editable now; notes and tags activate automatically after the client verifies a portal account.</p> : null}<label className="mt-5 block"><span className="form-label">New note</span><textarea className="form-control min-h-28" value={note} onChange={(event) => setNote(event.target.value)} /></label><div className="mt-3 flex flex-wrap gap-2"><select className="form-control max-w-48" value={visibility} onChange={(event) => setVisibility(event.target.value)}><option value="internal">Internal only</option><option value="client">Client visible</option></select><button type="button" disabled={busy || !note.trim() || !client.authUserId} onClick={() => void action({ action: "add_note", note, visibility }, "Client note added.")} className="rounded-full bg-[var(--color-brass)] px-5 py-3 text-[9px] tracking-[.16em] uppercase text-[var(--color-ink)] disabled:opacity-50">Add note</button></div><div className="mt-5 flex gap-2"><input className="form-control" value={tag} onChange={(event) => setTag(event.target.value)} placeholder="Tag" /><button type="button" disabled={busy || !tag.trim() || !client.authUserId} onClick={() => void action({ action: "add_tag", tag }, "Client tag added.")} className="rounded-full border border-[var(--color-ink-line)] px-5 text-[9px] tracking-[.14em] uppercase disabled:opacity-50">Add tag</button></div></section>
