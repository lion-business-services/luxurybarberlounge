...
import { loadTodayRevenue } from "@/lib/queue/payments";
import styles from "./admin-portal.module.css";

const money = (cents: number) => new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(cents / 100);

export async function AdminDashboard() {
  const [data, revenue] = await Promise.all([loadAdminPortalData(), loadTodayRevenue()]);
    ...
    <div className={styles.grid}>
      <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-[var(--color-bone-muted)]">One clear place to run appointments, walk-ins, clients, barbers, services, memberships, and commissions.</p>
        </div>
      ...
      <section className={`${styles.grid} ${styles.gridTwo}`}>
        <article className={styles.card}>
          <div className={styles.toolbar}>
            <div><p className="text-[9px] tracking-[.2em] uppercase text-[var(--color-brass)]">Live walk-ins</p><h2 className="font-display mt-2 text-2xl">Current queue</h2></div>
            <div className="flex flex-wrap gap-3"><Link href="/queue-board" target="_blank" className="text-[9px] tracking-[.15em] uppercase text-[var(--color-bone-muted)]">Open TV display</Link><Link href="/admin/queue" className="text-[9px] tracking-[.15em] uppercase text-[var(--color-brass)]">Manage queue</Link></div>
          </div>
          {data.queue.length ? (
            <div>{data.queue.slice(0, 5).map((row) => (
              <div key={String(row.id)} className={styles.queueItem}>
                <div><strong className="text-sm">{String(row.client_name ?? `Guest ${String(row.public_token ?? "").slice(-4)}`)}</strong><p className="mt-1 text-xs text-[var(--color-bone-muted)]">{String(row.service_slug ?? "Service pending").replaceAll("-", " ")} · {dateTime(String(row.joined_at))}</p></div>
                <div className="flex items-center gap-2"><span className="rounded-full bg-white/[.04] px-2 py-1 text-[8px] tracking-[.12em] uppercase text-[var(--color-brass)]">{titleCase(String(row.status))}</span><span className="text-xs text-[var(--color-bone-muted)]">{typeof row.estimated_wait_minutes === "number" ? `${row.estimated_wait_minutes} min` : "Pending"}</span></div>
              </div>
            ))}</div>
      ...
      <section>
        <div className={styles.toolbar}><div><p className="text-[9px] tracking-[.2em] uppercase text-[var(--color-brass)]">Quick controls</p><h2 className="font-display mt-2 text-2xl">Run the shop</h2></div></div>
        <div className={styles.actionGrid}>
          <AdminAction href="/admin/appointments" icon={CalendarDays} title="Appointments" copy="Schedule and status" />
          <AdminAction href="/admin/queue" icon={ClipboardList} title="Queue" copy="Walk-ins, payments, receipts" />
          ...
          <AdminAction href="/admin/commissions" icon={CircleDollarSign} title="Commissions" copy="Weekly pay and statements" />
        </div>
      </section>
    </div>
  );
...
function Metric({ label, value, note }: { label: string; value: string; note: string }) {
  return <article className={styles.metric}><p className="text-[8px] tracking-[.16em] uppercase text-[var(--color-bone-muted)]">{label}</p><p className={styles.metricValue}>{value}</p><p className="mt-1 text-[10px] text-[var(--color-bone-muted)]">{note}</p></article>;
}

function Pulse({ label, value }: { label: string; value: string }) {
  return <div className={styles.pulseItem}><span className="text-xs text-[var(--color-bone-muted)]">{label}</span><strong className="text-xs text-[var(--color-bone)]">{value}</strong></div>;
}
...
function AdminAction({ href, icon: Icon, title, copy }: { href: string; icon: React.ComponentType<{ className?: string }>; title: string; copy: string }) {
  return <Link href={href} className={styles.actionCard}><Icon className="h-5 w-5 text-[var(--color-brass)]" /><div><h3 className="font-display text-lg">{title}</h3><p className="mt-1 text-[11px] leading-4 text-[var(--color-bone-muted)]">{copy}</p></div></Link>;
}
