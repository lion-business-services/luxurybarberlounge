...
const paymentStatusLabel = (value: string) => value === "unmatched" ? "Needs review" : pretty(value);
...
export function AdminPaymentTracking() {
  ...
  const load = useCallback(async () => {
    ...
    if (!response?.ok || !result?.ok) {
      setMessage("Payment tracking could not be loaded. Please refresh and try again.");
      return;
    }
    setPayload(result);
    setMessage("");
  ...
  if (!payload) return <div className="rounded-2xl border border-[var(--color-ink-line)] p-8 text-sm text-[var(--color-bone-muted)]">Loading payment records…</div>;

  return <div className="grid gap-6">
    <header className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      <div><p className="text-[10px] uppercase tracking-[.24em] text-[var(--color-brass)]">Payments</p><h1 className="font-display mt-2 text-4xl sm:text-5xl">Payment Tracking</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--color-bone-muted)]">Review walk-in and appointment payments in one place, separate from commissions. Payment records update automatically.</p></div>
    ...
    <section className="grid gap-3 md:grid-cols-4">
      <Metric label="Paid total" value={money(payload.summary.totalPaidCents)} />
