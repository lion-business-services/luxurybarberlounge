...
const copy: Record<string, { eyebrow: string; title: string; description: string; icon: React.ComponentType<{ className?: string }> }> = {
  ...
  appointments: { eyebrow: "Booking operations", title: "Appointments", description: "Square-synchronized bookings with Supabase extensions and status history.", icon: CalendarDays },
  services: { eyebrow: "Catalog", title: "Services", description: "Service definitions, pricing versions, duration, deposits, and provider mappings.", icon: Scissors },
  ...
  commissions: { eyebrow: "Commission governance", title: "Calculated Amounts", description: "Commission Basis, tips, adjustments, integrity flags, and locked calculations.", icon: CircleDollarSign },
  statements: { eyebrow: "Settlement", title: "Statements", description: "Weekly statement periods, delivery, disputes, lock state, and manual settlement status.", icon: FileText },
...
export async function AdminMembershipsPage() {
  ...
  return <AdminPageHeader eyebrow="Membership operations" title="Memberships" copy="Plan versioning, status, usage, renewal communication, and provider-confirmed billing state.">
    ...
    {data.memberships.length ? <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Membership</th><th>Client</th><th>Plan</th><th>Status</th><th>Renews</th></tr></thead><tbody>{data.memberships.map((item) => <tr key={item.id}><td>{item.id.slice(0, 8)}</td><td><Link href={`/admin/clients/${item.clientId}`} className="text-[var(--color-brass)]">{item.clientId.slice(0, 8)}</Link></td><td>{item.plan}</td><td>{titleCase(item.status)}</td><td>{item.renewsAt ? shortDate(item.renewsAt) : "—"}</td></tr>)}</tbody></table></div> : <Empty text="No client memberships exist. Draft plans may be configured without implying billing is active." />}
  </AdminPageHeader>;
}
...
export async function AdminBarbersPage() {
  const [data, session] = await Promise.all([loadAdminPortalData(), getServerAuthSession()]);
  const owner = session.roles.some((role) => role === "owner" || role === "super_admin");
  return <AdminPageHeader eyebrow="Barber operations" title="Barbers" copy="Profiles, public visibility, specialties, services, availability, Square mapping, attribution, and Statements." action={owner ? { href: "/admin/users", label: "Invite barber" } : undefined}>
    {data.barbers.length ? <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{data.barbers.map((barber) => <article key={barber.id} className={styles.card}><div className="flex items-start justify-between gap-3"><div><p className="text-[8px] tracking-[.16em] uppercase text-[var(--color-brass)]">{barber.title}</p><h2 className="font-display mt-2 text-2xl">{barber.name}</h2></div><span className="rounded-full bg-white/[.04] px-2 py-1 text-[8px] uppercase tracking-[.12em] text-[var(--color-brass)]">{barber.active ? titleCase(barber.status) : "Inactive"}</span></div><div className="mt-5 flex gap-4"><Link href={`/admin/barbers/${barber.id}`} className="text-[9px] tracking-[.14em] uppercase text-[var(--color-brass)]">Manage</Link><Link href={`/barbers/${barber.slug}`} className="text-[9px] tracking-[.14em] uppercase">Public profile</Link></div></article>)}</div> : <Empty text="No database-backed barber profiles exist yet. Public static profiles remain unchanged." />}
  </AdminPageHeader>;
}
...
export async function AdminBarberDetail({ id }: { id: string }) {
  ...
  const owner = session.roles.some((role) => role === "owner" || role === "super_admin");
  return <AdminPageHeader eyebrow="Barber operations" title={barber?.name ?? "Barber record"} copy="Profile, services, schedule, assignments, attribution, Calculated Amounts, Statements, and policy acknowledgment.">
    {barber ? <div className="grid gap-4"><section className={styles.metricGrid}>{Object.entries(barber.totals).map(([label,value]) => <article key={label} className={styles.metric}><p className="text-[8px] tracking-[.17em] uppercase text-[var(--color-bone-muted)]">{titleCase(label)}</p><p className={styles.metricValue}>{value}</p><span className={styles.source}>Supabase-derived</span></article>)}</section><div className="grid gap-4 lg:grid-cols-3"><section className={styles.card}><Field label="Title" value={barber.title} /><div className="mt-4"><Field label="Profile state" value={titleCase(barber.status)} /></div><div className="mt-4"><Field label="Active" value={barber.active ? "Yes" : "No"} /></div><div className="mt-4"><Field label="Square mapping" value={barber.squareTeamMemberId ?? "Not mapped"} /></div><div className="mt-4"><Field label="Languages" value={barber.languages.join(", ") || "Not recorded"} /></div></section><section className={`${styles.card} lg:col-span-2`}><h2 className="font-display text-2xl">Operating controls</h2><p className="mt-3 text-sm leading-6 text-[var(--color-bone-muted)]">{barber.intro || barber.biography || "No verified biography is stored yet."}</p>{barber.specialties.length ? <div className="mt-4 flex flex-wrap gap-2">{barber.specialties.map((item) => <span key={item} className="rounded-full bg-white/[.05] px-3 py-1 text-[9px] uppercase tracking-[.12em] text-[var(--color-brass)]">{item}</span>)}</div> : null}<div className="mt-5 grid gap-3 sm:grid-cols-3"><ModuleLink href={`/admin/appointments?barber=${barber.staffUserId ?? barber.id}`} title="Appointments" /><ModuleLink href="/admin/attribution" title="Attribution" /><ModuleLink href="/admin/statements" title="Statements" /></div></section></div><AdminBarberEditor barber={barber} owner={owner} /></div> : <Empty text="Barber not found." />}
  </AdminPageHeader>;
}
...
export async function AdminModulePage({ slug }: { slug: string }) {
  ...
  return <AdminPageHeader eyebrow={page.eyebrow} title={page.title} copy={page.description}>
    ...
    {snapshot.records.length ? <section className={styles.card}><div className="flex items-center gap-3"><Icon className="h-5 w-5 text-[var(--color-brass)]" /><h2 className="font-display text-2xl">Authorized records</h2></div><div className={`${styles.tableWrap} mt-5`}><table className={styles.table}><thead><tr><th>Record</th><th>Details</th><th>Status</th><th>Context</th></tr></thead><tbody>{snapshot.records.map((item) => <tr key={item.id}><td><strong>{item.primary}</strong><br /><span className="text-[9px] text-[var(--color-bone-muted)]">{item.id}</span></td><td>{item.secondary || "—"}</td><td>{titleCase(item.status)}</td><td>{item.meta || "—"}</td></tr>)}</tbody></table></div></section> : <div className="grid gap-4 lg:grid-cols-[1.1fr_.9fr]"><section className={styles.card}><div className="flex items-center gap-3"><Icon className="h-5 w-5 text-[var(--color-brass)]" /><h2 className="font-display text-2xl">No live records yet</h2></div><p className="mt-4 text-sm leading-6 text-[var(--color-bone-muted)]">This module is authorized and connected, but no provider-confirmed or Supabase-backed records are available. Demo records are not substituted.</p></section><section className={styles.card}><h2 className="font-display text-2xl">Data state</h2><p className="mt-3 text-sm leading-6 text-[var(--color-bone-muted)]">{data.configured ? "Supabase is configured. Records appear only when they exist and pass authorization." : "Supabase is not available for this request."}</p><div className="mt-5 grid gap-2">{owner ? <><ModuleLink href="/admin/audit" title="Audit trail" /><ModuleLink href="/admin/integrations" title="Integration health" /></> : <ModuleLink href="/admin/today" title="Return to operations" />}</div></section></div>}
  </AdminPageHeader>;
}
