...
function section(slug: string, data: BarberPortalData) {
  ...
  if (slug === "profile") return <div className="grid gap-5 lg:grid-cols-[18rem_1fr]"><section className="portal-card">{data.barber.portrait ? <Image src={data.barber.portrait} alt={data.barber.name} width={480} height={600} className="aspect-[4/5] w-full rounded-xl object-cover" /> : <div className="grid aspect-[4/5] place-items-center rounded-xl border border-dashed border-[var(--color-ink-line)] text-sm text-[var(--color-bone-muted)]">Profile photo not available</div>}<h2 className="font-display mt-5 text-2xl">{data.barber.name}</h2><p className="mt-2 text-sm text-[var(--color-bone-muted)]">{data.barber.title}</p>{data.barber.socialUrl ? <a href={data.barber.socialUrl} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 text-xs text-[var(--color-brass)]">{data.barber.socialHandle ? `@${data.barber.socialHandle}` : "Social profile"}<ExternalLink className="h-3.5 w-3.5" /></a> : null}</section><section className="portal-card"><div className="grid gap-4 sm:grid-cols-2"><ProfileFact icon={<Scissors />} label="Availability" value={titleCase(data.barber.availabilityStatus)} /><ProfileFact icon={<UsersRound />} label="Walk-ins" value={data.barber.acceptingWalkIns ? "Accepting" : "Not accepting"} /><ProfileFact icon={<CircleDollarSign />} label="Current statement" value={money(data.commission.currentAmountCents)} /><ProfileFact icon={<Clock3 />} label="Schedule rows" value={String(data.schedule.length)} /></div>{data.schedule.length ? <div className="mt-7"><PortalTable columns={["Day","Hours"]} rows={data.schedule.map((item) => ({ Day: weekday[item.weekday] ?? String(item.weekday), Hours: `${time(item.start)} - ${time(item.end)}` }))} /></div> : null}</section></div>;
  if (slug === "portfolio") return data.barber.portrait ? <div className="grid gap-5 lg:grid-cols-[18rem_1fr]"><section className="portal-card"><Image src={data.barber.portrait} alt={data.barber.name} width={480} height={600} className="aspect-[4/5] w-full rounded-xl object-cover" /></section><section className="portal-card"><p className="text-[9px] uppercase tracking-[.18em] text-[var(--color-brass)]">Published profile</p><h2 className="font-display mt-3 text-2xl">{data.barber.name}</h2><p className="mt-3 text-sm text-[var(--color-bone-muted)]">This is the approved professional profile image currently linked to your public barber identity.</p><div className="mt-5 flex flex-wrap gap-3">{data.barber.slug ? <Link href={`/barbers/${data.barber.slug}`} className="inline-flex items-center gap-2 text-xs text-[var(--color-brass)]">Open public profile <ExternalLink className="h-3.5 w-3.5" /></Link> : null}{data.barber.socialUrl ? <a href={data.barber.socialUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-xs text-[var(--color-brass)]">{data.barber.socialHandle ? `@${data.barber.socialHandle}` : "Social profile"}<ExternalLink className="h-3.5 w-3.5" /></a> : null}</div></section></div> : <EmptyState title="No published profile media" copy="A public profile image has not been published for this barber account yet." />;
