...
export function WalkInFlow({ kiosk = false }: { kiosk?: boolean }) {
  ...
  return <form onSubmit={submit} className={kiosk?"mx-auto max-w-3xl":"border border-[var(--color-ink-line)] bg-[var(--color-ink-soft)]/70 p-6 sm:p-8"}>
    ...
    <label className="mt-6 flex items-start gap-3 text-xs leading-6 text-[var(--color-bone-muted)]"><input name="smsConsent" value="yes" type="checkbox" className="mt-1 accent-[var(--color-brass)]"/><span>I agree to transactional text messages about queue status. Message and data rates may apply.</span></label>
