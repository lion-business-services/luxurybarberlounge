...
export function SquareDepositButton({ reference, token, amountCents, status }: { reference: string; token: string; amountCents: number; status: string }) {
  ...
  return <div className="mt-6 rounded-xl border border-[var(--color-brass)]/25 bg-black/15 p-5"><p className="text-[9px] uppercase tracking-[.18em] text-[var(--color-brass)]">Required deposit</p><p className="font-display mt-2 text-3xl">{new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(amountCents / 100)}</p><p className="mt-2 text-xs leading-6 text-[var(--color-bone-muted)]">Secure payment is completed through Square and automatically linked back to this appointment.</p><button type="button" disabled={busy} onClick={() => void pay()} className="mt-4 inline-flex min-h-12 items-center gap-2 rounded-full bg-[var(--color-brass)] px-5 text-[10px] tracking-[.16em] uppercase text-black disabled:opacity-50"><CreditCard className="h-4 w-4" />{busy ? "Opening Square..." : "Pay deposit with Square"}</button>{message ? <p role="status" className="mt-3 text-xs text-amber-100">{message}</p> : null}</div>;
}
